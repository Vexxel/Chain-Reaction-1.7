...
public class TEHeatExchanger extends TEMultiBlockBase implements IFluidHandler, IThermalTile {
    ...
    private void processCoolant() {
        ...
        if (amount <= 0) return;
        if (output == null) {
            ZederrianEngineering.log.warn("Input heating fluid has no output in exchanger at " + getStringLocale() + "!");
    ...
    private void heatFluid() {
        ...
        if (output == null) {
            ZederrianEngineering.log.warn("Input working fluid has no output in exchanger at " + getStringLocale() + "!");
            ZederrianEngineering.log.warn("How did that fluid get there anyway? I'll purge that tank.");
    ...
    public void initiateController(UUID id, EntityPlayer player) {
        ...
        if (!this.isMaster && !this.hasValidMaster())
            if (checkMultiblock(player))  {
                ...
                ZederrianEngineering.proxy.playSound(this.worldObj, xCoord, yCoord, zCoord, Reference.Sounds.LOCK_SUCCESS, 1.0F, 1.0F);
            }
    }

...
/**
 * Interface for grouping multi-block structures to a single heat net delegate.
 * 
 * The heat net uses TileEntity to refer to a specific xyz+world position. If multiple of those
 * positions should belong to the same functional structure, i.e. they consume or produce heat
 * only once for the whole multi-block instead of once per every single block, this interface
 * allows to do so.
 * 
 * The tile entity implementing IMetaDelegate has to be added/removed to/from the heat net
 * instead of every single sub-TileEntity. The heat net interaction will be handled by the
